const dataJS = [
  ...
  },
  {
    ...
    answer:
      "Функція конструктор: Це звичайна JavaScript-функція, яка викликається за допомогою ключового слова new. Властивості і методи об'єкта, створеного за допомогою функції конструктора, додаються до this. Класи використовують спеціальний синтаксис і ключове слово class для оголошення класу. Властивості та методи класу визначаються всередині тіла класу. Обидва підходи призначені для створення об'єктів, але класи надають більше можливостей для структуризації коду та організації спадкування, що робить їх більш сучасним та потужним механізмом для роботи з об'єктами в JavaScript.",
  },
  {
    question:
      "Що потрібно зробити, щоб метод класа потрапив до його екземпляру?",
    answer:
    ...
    // запуститиДвигун() {
    //     console.log(`Двигун автомобіля ${this.марка} ${this.модель} запущено.`);
    // }
    // }
    //     const авто1 = new Автомобіль('Toyota', 'Camry');
    // const авто2 = new Автомобіль('Honda', 'Civic');

    // авто1.запуститиДвигун(); // Виведе "Двигун автомобіля Toyota Camry запущено."
    // авто2.запуститиДвигун(); // Виведе "Двигун автомобіля Honda Civic запущено."
  },
  {
    question: "Що таке LocalStorage та SessionStorage?",
    answer:
      "Дані, збережені в LocalStorage, залишаються на стороні клієнта після закриття браузера та перезавантаження сторінки.Дані в LocalStorage доступні для всіх вкладок (табів) в браузері, які відкриті для даного домену. Дані, збережені в SessionStorage, існують лише протягом одного сеансу роботи з браузером. Вони будуть втрачені після закриття вкладки браузера або виходу з сеансу.Дані в SessionStorage недоступні для інших вкладок (табів) в тому ж браузері і не переходять між сеансами.",
  },
  {
    question: "Що таке Promise?",
    answer:
      "Promise (обіцянка) - це об'єкт у JavaScript, який використовується для управління асинхронними операціями. Він представляє результат або стан обчислення, яке може бути успішним (resolved), помилковим (rejected) або в стані очікування (pending). Використання Promise дозволяє структурувати асинхронний код та керувати обробкою результатів асинхронних операцій (як приклад, бібліотека axios).  Стани Promise:pending (очікування): Початковий стан Promise, коли операція ще не завершилася і результат не відомий. resolved (успіх): Стан, в якому операція завершилася успішно, і результат доступний. rejected (відхилення): Стан, в якому операція завершилася з помилкою, і результат містить інформацію про помилку. Методи обробки результатів:then(): Метод, який викликається при успішному виконанні Promise. Він приймає функцію з аргументами, які представляють результат операції.  catch(): Метод, який викликається при виникненні помилки під час виконання Promise. Він приймає функцію для обробки помилок.",
  },
  {
    question: "Для чого потрібен метод Promise.all та Promise.race?",
    answer:
      "Метод Promise.all - очікує що всі переданні проміси будуть вирішені успішно(чекає завершеннявсіх промісів в группі). Якщо хочаб один проміс в групі поверне помилку то метод поверне перший відхилений проміс. Метод Promise.race очікує що хоаб один з переданних проісів буде вирішений. Він повертає результат перший завершений проміс.",
  },
  {
  ...
  },
  {
    ...
    answer: "Для цього можна використовувати конструкцію try...catch() ",
  },
  {
